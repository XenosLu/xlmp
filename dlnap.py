...
class DlnapDevice:
   ...
   def _create_packet_rcs(self, action, data):
      """ Create packet to send to device control url.
      ...
      packet = "\r\n".join([
         'POST {} HTTP/1.1'.format('/upnp/service/RenderingControl/Control'),
         'User-Agent: {}/{}'.format(__file__, __version__),
         'Accept: */*',
         'Content-Type: text/xml; charset="utf-8"',
         'HOST: {}:{}'.format(self.ip, self.port),
   ...
   def seek(self, progress, instance_id = 0):
      """
      xenos
      """
      packet = self._create_packet('Seek', {'InstanceID':instance_id, 'Unit':'REL_TIME', 'Target':progress })
      _send_tcp((self.ip, self.port), packet)

   def volume(self, vol, instance_id = 0):
      """
      xenos
      """#SETMUTE
